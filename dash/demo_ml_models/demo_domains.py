...
def extract_features(data):
    ...
    # Corrected 'class' to 'Label'
    return data[['Length', 'ContainsDigits', 'ContainsHyphen']], data['Label']
...
def visualize_length_distribution(data):
    ...
    """
    data['Label'] = data['Label'].map({0: 'Malicious', 1: 'Legit'})
    # Corrected 'class' to 'Label'
    sns.histplot(data=data, x='Length', hue='Label', kde=True, bins=20)
...
def visualize_tld_distribution(data):
    ...
    # Corrected 'class' to 'Label'
    ax = sns.countplot(data=data, x='TLD', hue='Label', order=data['TLD'].value_counts().index[:10])
    ...
    legend_labels = ['Malicious', 'Legit']  # Replace with your desired labels
...
def main():
    # Load data
    data = load_data("../demo_datasets/legit_malware_domains.csv")\
    ...
    # Removed redundant code for retraining the classifier and plotting the confusion matrix again
